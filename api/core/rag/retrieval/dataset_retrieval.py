...
class DatasetRetrieval:
    ...
    def retrieve(
        ...
        all_documents = []
        user_from = "account" if invoke_from in {
            InvokeFrom.EXPLORE, InvokeFrom.DEBUGGER} else "end_user"
        ...
        for item in external_documents:
            document_context_list.append(DocumentContext(
                ...
                for record in records:
                    ...
                    if segment.answer:
                        document_context_list.append(
                            DocumentContext(
                                content=f"question:{segment.get_sign_content()} answer:{
                                    segment.answer}",
                                score=record.score,
                            )
                        )
        ...
        if hit_callback and retrieval_resource_list:
            retrieval_resource_list = sorted(
                retrieval_resource_list, key=lambda x: x.get("score") or 0.0, reverse=True)
            for position, item in enumerate(retrieval_resource_list, start=1):
                item["position"] = position
            hit_callback.return_retriever_resource_info(
                retrieval_resource_list)
        if document_context_list:
            document_context_list = sorted(
                document_context_list, key=lambda x: x.score or 0.0, reverse=True)
                ...
                if dataset.provider == "external":
                    ...
                    for external_document in external_documents:
                        ...
                        if document.metadata is not None:
                            document.metadata["score"] = external_document.get(
                                "score")
                            document.metadata["title"] = external_document.get(
                                "title")
                    ...
                    if score_threshold_enabled:
                        score_threshold = retrieval_model_config.get(
                            "score_threshold", 0.0)
                    ...
                    with measure_time() as timer:
                        results = RetrievalService.retrieve(
                            ...
                            weights=retrieval_model_config.get(
                                "weights", None),
                        )
                ...
                if dataset.indexing_technique == "economy":
                    ...
                    if top_k > 0:
                        ...
                        documents = RetrievalService.retrieve(
                            ...
                            score_threshold=retrieval_model.get(
                                "score_threshold", 0.0)
                            if retrieval_model["score_threshold_enabled"]
                            else 0.0,
                            reranking_model=retrieval_model.get(
    ...
    def calculate_keyword_score(self, query: str, documents: list[Document], top_k: int) -> list[Document]:
        ...
        for keyword in all_keywords:
            ...
            # IDF
            keyword_idf[keyword] = math.log(
                (1 + total_documents) / (1 + doc_count_containing_keyword)) + 1
        ...
        documents = sorted(documents, key=lambda x: x.metadata.get(
