...
class DatasetMultiRetrieverTool(DatasetRetrieverBaseTool):
    ...
    def _run(self, query: str) -> str:
        ...
        for item in all_documents:
            if item.metadata and item.metadata.get("score"):
                document_score_list[item.metadata["doc_id"]
                                    ] = item.metadata["score"]

        document_context_list = []
        index_node_ids = [document.metadata["doc_id"]
                          for document in all_documents if document.metadata]
        ...
        if segments:
            ...
            sorted_segments = sorted(
                segments, key=lambda segment: index_node_id_to_position.get(
    ...
    def _retriever(
        ...
        with flask_app.app_context():
            dataset = (
                db.session.query(Dataset).filter(
                    Dataset.tenant_id == self.tenant_id, Dataset.id == dataset_id).first()
            )
