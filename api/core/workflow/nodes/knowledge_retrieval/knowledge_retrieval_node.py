...
class KnowledgeRetrievalNode(BaseNode[KnowledgeRetrievalNodeData]):
    ...
    def _run(self) -> NodeRunResult:
        # `knowledge_id` を取得
        knowledge_id_variable = self.graph_runtime_state.variable_pool.get(
            # "knowledgeId" の value_selector を取得
            self.node_data.variables[0]["value_selector"]
        )

        # `knowledge_id` の値を dataset_ids に追加
        if knowledge_id_variable:
            self.node_data.dataset_ids.append(knowledge_id_variable.value)
